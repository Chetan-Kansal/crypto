...
The algorithms were evaluated on their Key Generation time, Key Encapsulation time, and Data Encryption time. Tests were run on standardized file sizes: 1-Page text (~6 KB), 10-Page text (~33 KB), 50-Page text (~151 KB), 500-Page text (~1.5 MB), and a Multimedia Image (~1 MB). 

\subsection{Results}
Our benchmarking revealed that the symmetric encryption time (AES) scales linearly with the file size, peaking at roughly ~3 MB of memory usage for a 1.5 MB text file. The asymmetric operations remained constant regardless of file size.
